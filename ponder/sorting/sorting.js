...
// // Simliar to Homework (recipes part 2)
...
//     return tags.map((tag) => `<button>${tag}</buttom>`).join(' ');
// }
...
//             <h2>${hike.name}</h2>
//             <div class="hike-tag">
//                 ${tagTemplate(hike.tag)}
//             </div>
//             <p>${hike.description}</p>
//             <div class="hike-difficulty"></div>
//         </div>
//     </div>`
// }
...
// function init() {
//     renderHikes(hikes[random]);
// }

// init();


// ... hikes array and simpleList as provided
...
let anotherSort = simpleList.slice().sort(compareFn); // use slice to not sort in-place

let nums = [10, 12, 9, 8, 1];
let sortNums = nums.slice().sort((a,b)=>a-b);
console.log(sortNums);

// FILTERING
let query = 'ap'; // test query
let filterList = simpleList.filter(item => item.toLowerCase().includes(query.toLowerCase()));
console.log(filterList);
...
function renderHikes(hikesArray) {
    hikeContainer.innerHTML = hikesArray.map(hikeTemplate).join('');
}
...
function search() {
    ...
    filteredHikes.sort((a, b) => a.name.localeCompare(b.name));
...
// INIT
function init() {
    renderHikes(hikes); // render all to start
}
init();